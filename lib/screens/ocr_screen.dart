...
class _OcrScreenState extends State<OcrScreen> {
  ...
  Future<void> _exportAsTxt() async {
    if (_combinedText.isEmpty || _exportingTxt) return; // Prevent double tapping / duplicate creation.
    setState(() => _exportingTxt = true);
    try {
      final path = await runCreationFlow<String>(
        context: context,
        loadingTitle: 'Saving text…',
        loadingSubtitle: 'Exporting the recognized text as a .txt file.',
        successTitle: 'Text File Created Successfully',
        task: () async {
          final exportedPath =
              await OcrService.exportToTxt(_combinedText, outputName: 'OCR_${DateTime.now().millisecondsSinceEpoch}');
          await context.read<LibraryProvider>().registerFile(exportedPath);
          return exportedPath;
        },
      );
      if (!mounted) return;
      setState(() => _exportingTxt = false);
  ...
  Future<void> _exportAsPdf() async {
    if (_pageResults.isEmpty || _exportingPdf) return; // Prevent double tapping / duplicate creation.
    setState(() => _exportingPdf = true);
    try {
      final path = await runCreationFlow<String>(
        context: context,
        loadingTitle: 'Creating PDF…',
        loadingSubtitle: 'Exporting the recognized text as a PDF.',
        task: () async {
          final exportedPath = await OcrService.exportToPdf(
            _pageResults,
            _language,
              ...
              children: [
                Expanded(
                  child: SecondaryButton(
                    ...
                    onPressed: _exportingTxt ? null : _exportAsTxt,
                  ),
                ),
                const SizedBox(width: 12),
                Expanded(
