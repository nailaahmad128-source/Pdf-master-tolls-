...
class _LockUnlockPdfScreenState extends State<LockUnlockPdfScreen> {
  ...
  Future<void> _run() async {
    ...
    if (_busy) return; // Prevent double tapping / duplicate creation.
    setState(() => _busy = true);
    try {
      await runCreationFlow<String>(
        context: context,
        loadingTitle: _mode == _Mode.lock ? 'Locking PDF…' : 'Unlocking PDF…',
        loadingSubtitle: 'Please wait, this will only take a moment.',
        successTitle: _mode == _Mode.lock ? 'PDF Locked Successfully' : 'PDF Unlocked Successfully',
        task: () async {
