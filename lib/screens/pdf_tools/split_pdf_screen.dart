...
class _SplitPdfScreenState extends State<SplitPdfScreen> {
  ...
  List<HistoryEntry> _readHistory() =>
      LocalStore.instance.readBucket(StoreKeys.splitHistory).map(HistoryEntry.fromJson).toList();

  Future<void> _saveToHistory(String sourceName, List<String> outputPaths) async {
    final entry = HistoryEntry(
      id: '${DateTime.now().microsecondsSinceEpoch}',
      title: sourceName,
      subtitle: '${outputPaths.length} file${outputPaths.length == 1 ? '' : 's'} created',
      payload: outputPaths.join(HistoryEntry.pathDelimiter),
      createdAt: DateTime.now(),
    );
    await LocalStore.instance.pushToBucket(StoreKeys.splitHistory, entry.toJson(), maxItems: 100);
    if (!mounted) return;
    setState(() => _history = _readHistory());
  }

  Future<void> _clearHistory() async {
    await LocalStore.instance.clearBucket(StoreKeys.splitHistory);
    setState(() => _history = []);
  }
  ...
  Future<void> _split() async {
    ...
    if (_splitting) return; // Prevent double tapping / duplicate creation.
    setState(() => _splitting = true);
    final sourceName = _path!.split(Platform.pathSeparator).last;
    try {
      await runCreationFlow<List<String>>(
        context: context,
        loadingTitle: 'Splitting PDF…',
        loadingSubtitle: 'Breaking "$sourceName" into separate files.',
        ...
        task: () async {
          ...
          await _saveToHistory(sourceName, outputs);
          return outputs;
        },
      );
      if (!mounted) return;
              ...
              ? ListView(
                  children: [
                    ...
                    HistorySection(
                      ...
                      entries: _history,
                      emptyMessage: 'PDFs you split will show up here.',
                      onClear: _clearHistory,
                      historyBucketKey: StoreKeys.splitHistory,
