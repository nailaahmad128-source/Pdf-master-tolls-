...
class _ImageToolsScreenState extends State<ImageToolsScreen> {
  ...
  List<HistoryEntry> _readHistory() =>
      LocalStore.instance.readBucket(StoreKeys.imageToPdfHistory).map(HistoryEntry.fromJson).toList();

  Future<void> _saveToHistory(String outputPath) async {
    final entry = HistoryEntry(
      id: '${DateTime.now().microsecondsSinceEpoch}',
      title: outputPath.split(Platform.pathSeparator).last,
      subtitle: '${_imagePaths.length} image${_imagePaths.length == 1 ? '' : 's'}',
      payload: outputPath,
      createdAt: DateTime.now(),
    );
    await LocalStore.instance.pushToBucket(StoreKeys.imageToPdfHistory, entry.toJson(), maxItems: 100);
    if (!mounted) return;
    setState(() => _history = _readHistory());
  }

  Future<void> _clearHistory() async {
    await LocalStore.instance.clearBucket(StoreKeys.imageToPdfHistory);
    setState(() => _history = []);
  }
  ...
  Future<void> _createPdf() async {
    ...
    if (_creating) return; // Prevent double tapping / duplicate creation.
    ...
    try {
      await runCreationFlow<String>(
        ...
        loadingSubtitle: 'Combining $imageCount image${imageCount == 1 ? '' : 's'} into a PDF.',
        successSubtitle: '$imageCount image${imageCount == 1 ? '' : 's'} saved as a PDF.',
        task: () async {
          ...
          await _saveToHistory(outputPath);
          return outputPath;
        },
      );
      if (!mounted) return;
        ...
        child: ListView(
          ...
          children: [
            ...
            HistorySection(
              ...
              entries: _history,
              emptyMessage: 'PDFs you create from images will show up here.',
              onClear: _clearHistory,
              historyBucketKey: StoreKeys.imageToPdfHistory,
