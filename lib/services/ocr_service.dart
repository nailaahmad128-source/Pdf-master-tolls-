...
/// Handles single- and multi-page OCR across two backends:
///
...
///
/// - Tesseract (`flutter_tesseract_ocr`) for Arabic and Urdu. Tesseract's
///   `.traineddata` files are ~1-15MB each and are NOT bundled in the
///   APK to keep install size down; the first time a script is used,
///   [ensureLanguageReady] downloads it once from the official tessdata
///   mirror and caches it under app documents. Every OCR call after that
///   is fully offline, same as ML Kit.
class OcrService {
  OcrService._();
  ...
  static Future<String> recognizeImage(String imagePath, OcrLanguage language) async {
    ...
    final code = language.tesseractCode!;
    if (!await isLanguageReady(language)) {
      throw OcrException(
        '${language.label} OCR language pack is not downloaded yet. Call ensureLanguageReady() first.',
      );
    }
    final tessdataDir = await _tessdataDir();
    try {
      final text = await FlutterTesseractOcr.extractText(
        ...
        args: {
          ...
          // PSM 6: "assume a single uniform block of text" -- the best
          // general-purpose mode for photographed documents/pages as
          // opposed to single lines or sparse text.
          'psm': '6',
        },
      );
      return text.trim();
    } catch (e) {
      throw OcrException('Could not read ${language.label} text from this image: $e');
    }
  }
