...
class LibraryProvider extends ChangeNotifier {
  ...
  /// Duplicates a file and registers the copy as a new recent entry,
  /// leaving the original untouched.
  Future<LibraryFile> copyFile(String path) async {
    final newPath = await FileService.copyFile(path);
    return registerFile(newPath);
  }

  /// Moves a file to a folder outside the app's working directory and
  /// updates any Recent/Favorite entries pointing at its old path so
  /// they don't go stale.
  Future<void> moveFile(String path, String destinationDir) async {
    final newPath = await FileService.moveFile(path, destinationDir);
    await _replacePath(StoreKeys.recentFiles, path, newPath);
    await _replacePath(StoreKeys.favoriteFiles, path, newPath);
    _load();
    notifyListeners();
  }
